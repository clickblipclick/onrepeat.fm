...
const okDeps: ResolverDeps = {
  ...
  youtube: {
    ...
    async lookupVideos() { return new Map([['yt1', { durationSec: 201, embeddable: true }]]) },
  },
...
describe('resolveJob (v2)', () => {
  ...
  it('falls back to the source oEmbed cover when iTunes has no match', async () => {
    ...
    const deps: ResolverDeps = {
      itunes: { async search() { return [] }, async lookup() { return null } },
      oembed: async () => ({ thumbnail: 'https://i.scdn.co/image/cover.jpg' }),
    }
    await resolveJob(db, deps, { identity: 'ta:a|c', sourceUrl: 'https://open.spotify.com/track/sp1', provider: 'spotify' })
    const t = await db.selectFrom('tracks').selectAll().where('id', '=', 'ta:a|c').executeTakeFirst()
  ...
  it('prefers the iTunes cover over the source oEmbed cover', async () => {
    await seedPending('ta:a|c')
    let fetched = false
    const deps: ResolverDeps = { ...okDeps, oembed: async () => { fetched = true; return { thumbnail: 'https://i.scdn.co/image/oembed.jpg' } } }
    await resolveJob(db, deps, { identity: 'ta:a|c', sourceUrl: 'https://open.spotify.com/track/sp1', provider: 'spotify' })
    const t = await db.selectFrom('tracks').selectAll().where('id', '=', 'ta:a|c').executeTakeFirst()
  ...
  it('bandcamp: writes the scraped cover art onto the track', async () => {
    ...
    const deps = { ...okDeps, bandcamp: async () => ({ trackId: '999', artworkUrl: 'https://f4.bcbits.com/img/a1_10.jpg' }) }
    await resolveJob(db, deps, { identity: 'ta:b|art', sourceUrl: 'https://x.bandcamp.com/track/y', provider: 'bandcamp' })
    const t = await db.selectFrom('tracks').selectAll().where('id', '=', 'ta:b|art').executeTakeFirst()
