...
describe('getActorJams + getFollowFeed', () => {
  ...
  it('getActorJams returns the actor jams newest-first (current + archive)', async () => {
    ...
    await insertJam(
      'at://did:plc:a/fm.onrepeat.feed.jam/2',
      'did:plc:a',
  ...
  it("getFollowFeed includes the viewer's own current jam, sorted chronologically (no self-follow)", async () => {
    ...
    await insertJam(
      'at://did:plc:me/fm.onrepeat.feed.jam/cur',
      'did:plc:me',
      new Date(Date.now() - 1 * 60_000).toISOString(),
    ) // mine, newest
    const page = await getFollowFeed(db, {
      followedDids: ['did:plc:a'],
  ...
  it('getActorJams paginates by cursor and never leaks other authors', async () => {
    ...
    await insertJam(
      'at://did:plc:a/fm.onrepeat.feed.jam/3',
      'did:plc:a',
    ...
    const second = await getActorJams(db, {
      did: 'did:plc:a',
      limit: 2,
      cursor: first.cursor,
    })
    ...
    expect(
      [...first.jams, ...second.jams].every((j) => j.authorDid === 'did:plc:a'),
    ).toBe(true)
  })
  ...
  it('getFollowFeed pagination is stable when an author posts again mid-paging', async () => {
    ...
    await insertJam(
      'at://did:plc:b/fm.onrepeat.feed.jam/cur',
      'did:plc:b',
      new Date(Date.now() - 2 * 60_000).toISOString(),
    )
    await insertJam(
    ...
    await insertJam(
      'at://did:plc:b/fm.onrepeat.feed.jam/new',
      'did:plc:b',
  ...
  it('getFollowFeed orders current jams newest-first across authors and paginates', async () => {
    ...
    expect(first.jams.map((j) => j.uri)).toEqual([
      'at://did:plc:c/fm.onrepeat.feed.jam/cur', // newest
      'at://did:plc:b/fm.onrepeat.feed.jam/cur',
    ])
    expect(first.cursor).toBeTruthy()
    const second = await getFollowFeed(db, {
