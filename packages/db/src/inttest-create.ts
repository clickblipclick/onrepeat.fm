...
async function main(): Promise<void> {
  ...
  if (!name || name === 'onrepeat_test' || !/^[a-zA-Z0-9_]+$/.test(name)) {
    throw new Error(`[inttest] refusing to use '${name || url}' as the integration database.`)
  }
  ...
  try {
    const existing = await sql`select 1 from pg_database where datname = ${name}`.execute(admin)
...
main().catch((err) => {
  console.error('[inttest] failed to ensure database:', err instanceof Error ? err.message : err)
  process.exit(1)
})
