...
describe('deriveTrack', () => {
  it('apple url → iTunes lookup', async () => {
    const fetchFn = async (url: string) => {
      ...
      return { ok: true, status: 200, async json() { return { results: [{ trackName: 'T', artistName: 'A', artworkUrl100: 'https://x/100x100bb.jpg', trackViewUrl: 'https://music.apple.com/us/album/t/1?i=2' }] } }, async text() { return '' } }
    }
  ...
  it('youtube url → oEmbed, splitting "Artist - Title"', async () => {
    const fetchFn = async () => ({ ok: true, status: 200, async json() { return { title: 'Frank Ocean - Thinkin Bout You (Official)', author_name: 'FrankOceanVEVO', thumbnail_url: 'https://t/i.jpg' } }, async text() { return '' } })
  ...
  it('spotify url → oEmbed title + artist from page meta', async () => {
    const fetchFn = async (u: string) => {
      if (u.includes('/oembed')) return { ok: true, status: 200, async json() { return { title: 'Thinkin Bout You', thumbnail_url: 'https://t/i.jpg' } }, async text() { return '' } }
      return { ok: true, status: 200, async json() { return {} }, async text() { return '<meta name="music:musician_description" content="Frank Ocean">' } }
    }
    const r = await deriveTrack('https://open.spotify.com/track/x', { fetchFn })
    expect(r).toMatchObject({ title: 'Thinkin Bout You', artist: 'Frank Ocean', provider: 'spotify' })
  })
