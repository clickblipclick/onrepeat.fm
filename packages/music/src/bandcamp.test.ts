...
import { parseBandcampEmbedId, parseBandcampArtwork, fetchBandcampEmbed } from './bandcamp'
...
describe('parseBandcampArtwork', () => {
  ...
  it('handles reversed attribute order', () => {
    expect(parseBandcampArtwork(`<meta content="${ART}" property="og:image">`)).toBe(ART)
  })
  it('returns null when absent', () => {
...
describe('fetchBandcampEmbed', () => {
  it('returns the trackId and og:image artwork', async () => {
    ...
    expect(await fetchBandcampEmbed('https://x.bandcamp.com/track/y', { fetchFn })).toEqual({ trackId: '1234567890', artworkUrl: ART })
  })
  it('returns just the artwork when there is no embed id', async () => {
    const fetchFn = async () => ({ ok: true, status: 200, async text() { return `<meta property="og:image" content="${ART}">` } })
    expect(await fetchBandcampEmbed('https://x.bandcamp.com/track/y', { fetchFn })).toEqual({ artworkUrl: ART })
  })
